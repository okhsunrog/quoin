...
Выводы: quoin-Max берёт лучший ratio на **каждой decimal-колонке** и на
большинстве целых; на по-настоящему случайных ID (`WatchID`) все упираются в ~1.0×
(это не сжимается ничем), а `zstd -19` чуть обходит его на одной структурной i32
(`RegionID`). Однопоточно выигрыш quoin — это **ratio + скорость декода**: на decimal
`city_temperature` quoin-Balanced даёт 13.3×, декодируя ~2 ГБ/с, а quoin-Max доводит
до 17.4× — выше 14.3× у `zstd -19`. Та же оговорка, что и для float, действует на
кодировании: поиск кодека CPU-тяжёлый, поэтому быстрые LZ-базлайны кодируют быстрее
(при куда меньшем ratio), а quoin всё равно кодирует заметно впереди `zstd -19`.
