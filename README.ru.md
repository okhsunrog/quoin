...
`target-cpu=native`), **закреплено на 4 производительных ядрах** (`taskset -c 0-7`,
`RAYON_NUM_THREADS=8`) — чтобы числа были стабильны и не уезжали на E-ядра. Целые
колонки — реальные `i64`/`i32` из датасета ClickBench `hits`; float-колонки —
**бенчмарк-корпус ALP**; decimal-колонки — реальные f64-значения как fixed-point
`Decimal128`. Ratio = оригинал ÷ сжатое (больше — лучше); пропускная способность в
МБ/с *несжатых* данных (медиана из 3 прогонов). Базлайны (`lz4`, `zlib`, `zstd`) —
...
На каждом графике каждый кодек — одна точка; **верхний правый угол — лучший**
(высокий ratio, высокая пропускная способность), слева скорость сжатия, справа —
распаковки. Графики упорядочены по тому, насколько заметен отрыв quoin: **целые и
decimal сверху** (там специализированные лейны уходят дальше всего вперёд),
**float снизу** (самый трудный случай для quoin).

**Целые** — quoin-Balanced вверху-справа на *обеих* осях (выше ratio *и* быстрее
любого базлайна); quoin-Max растягивает ratio до 8.4×, тогда как `zstd -19`
достаёт лишь 5.1×, обвалившись до 3.3 МБ/с сжатия:

![целые колонки: ratio vs скорость](docs/images/pareto_int.png)

**Decimal** — quoin-Max берёт лучший ratio (17.4× против 14.3× у `zstd -19`) при
~15× скорости кодирования; quoin-Balanced меняет каплю ratio на ~40× более быстрое
кодирование и самую быструю распаковку:

![decimal колонки: ratio vs скорость](docs/images/pareto_decimal.png)

**Float** — самый узкий отрыв (биты мантиссы высокоэнтропийны), но quoin всё равно
вверху-справа: quoin-Balanced бьёт ratio `zstd -19` (2.53 против 2.20), сжимая
**~145× быстрее** и распаковывая **~3.6× быстрее**:
...
**~145× быстрее** и распаковывая **~3.6× быстрее**; quoin-Max доводит ratio до 2.72.
...
(`RegionID`). Но главная история — **разрыв по скорости при равном-или-лучшем
ratio**: на decimal `city_temperature` quoin-Balanced даёт 13.3× при **134 МБ/с**
сжатия против 14.3× у `zstd -19` при **3.2 МБ/с** — ~40× быстрее кодирование при
сравнимом ratio, а quoin-Max затем обходит его и по ratio. Относительное
преимущество по скорости — то же, что и на f64, но абсолютные ratio гораздо выше.
