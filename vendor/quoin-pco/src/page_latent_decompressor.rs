...
// Vendored-fork change: the offset bit-unpacking loop is one of pco's two
// vectorizable decode hot spots (~28% of decode on smooth f64). Upstream relies
// on a global `-C target-cpu=native` to vectorize it; we instead emit per-CPU
// clones (AVX2+BMI2 / AVX2 / SSE4.2 / NEON) dispatched at run time, so a stock
// (non-native) build still gets the fast path. The inner `read_uint_at`/`u64_at`
// are `#[inline]`, so they fold into each clone and recompile with its features.
#[multiversion(targets("x86_64+avx2+bmi2", "x86_64+avx2", "x86_64+sse4.2", "aarch64+neon"))]
#[inline(never)]
unsafe fn read_offsets<L: Latent, const READ_BYTES: usize>(
  ...
  for i in 0..n {
    ...
    // SAFETY: same contract as the enclosing `unsafe fn` — the caller guarantees
    // the offsets index valid bits of `src`. The explicit block is needed because
    // `#[multiversion]` emits the body into clone items that don't inherit the
    // outer fn's unsafety.
    let offset = unsafe {
      bit_reader::read_uint_at::<L, READ_BYTES>(src, byte_idx as usize, bits_past_byte, offset_bits)
    };
