...
// Vendored upstream code keeps its own lint standard; don't hold it to quoin's
// clippy style (newer clippy versions flag idioms upstream's CI predates).
#![allow(clippy::all)]
#![allow(clippy::uninit_vec)]
#![allow(clippy::manual_non_exhaustive)] // sometimes we want to ban explicit construction within the crate too
#![deny(clippy::unused_unit)]
// Upstream uses `#![deny(dead_code)]`; relaxed in the vendored fork because
// trimming (f16 removal, dropped compatibility tests) leaves some upstream
// helpers unreferenced. We don't want those to fail the build.
#![allow(dead_code)]
