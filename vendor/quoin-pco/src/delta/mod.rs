...
//
// Vendored-fork change: a trivially vectorizable map on the decode hot path;
// `#[multiversion]` emits per-CPU clones so a non-native build still vectorizes
// it (see `page_latent_decompressor::read_offsets` for the rationale).
