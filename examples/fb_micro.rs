...
fn main() {
    ...
    let cols: Vec<(&str, Vec<u64>)> = vec![
        ...
        (
            "bounded10+outliers",
            (0..n)
                .map(|i| if i % 500 == 7 { 1 << 40 } else { lcg() >> 54 })
                .collect(),
        ),
    ...
    for (name, v) in &cols {
        ...
        let enc = med(|| {
            black_box(bi::for_bitpack_encode(black_box(v)));
        });
        let dec = med(|| {
            black_box(bi::for_bitpack_decode(black_box(&e), v.len()).unwrap());
        });
        ...
        let enc = med(|| {
            black_box(bi::delta_bitpack_encode(black_box(v)));
        });
        let dec = med(|| {
            black_box(bi::delta_bitpack_decode(black_box(&e), v.len()).unwrap());
        });
